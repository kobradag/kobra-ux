...
export class KaspaWalletMobile extends KaspaWalletUI {
	...
	render() {
		...
		let donationAddresses = [
			["Devfund donations:", "kobra:qq5gtjz7xhghcyauyhwmy9a696ym7nhaj857t32l25qqysyzz27lzy9esv046"],
			["Mining address:", "kobra:qq5gtjz7xhghcyauyhwmy9a696ym7nhaj857t32l25qqysyzz27lzy9esv046"],
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">KOBRA WALLET</div>
				...
				return html`
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Kobra Core:</span> ${window.PWA_MODULES['@kaspa/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Kobra Wallet Framework:</span> ${window.PWA_MODULES['@kaspa/wallet']}</div>
							<div class="badge"><span is="i18n-span">Kobra gRPC:</span> ${window.PWA_MODULES['@kaspa/grpc']}</div>
							<div class="badge"><span is="i18n-span">Kobra gRPC Relay:</span> ${window.PWA_MODULES['@kaspa/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Kobra UX:</span> ${window.PWA_MODULES['@kaspa/ux']}</div>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">KOBRA FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip || "")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] KOBRA</b> available.', KAS(this.faucetFundsAvailable || 0))}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${KAS(inUseUTXOs.satoshis || 0)} KOBRA</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	openAddressExplorer(address) {
		if (!address)
			return
		let url = `https://explorer.kobradag.online/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatKAS(total) + ' KOBRA' : ''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatKAS(available)} KOBRA</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatKAS(pending)} KOBRA</span>
					</div>
				`}
            </div>
	...
	showReceiveDialog() {
		let address = this.receiveAddress || 'kobra:abc'
