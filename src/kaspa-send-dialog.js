...
class KaspaSendDialog extends KaspaDialog {
	...
	renderBody() {
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'kobra' prefix)`)}"
				value="${this.address || ''}"
				placeholder="">
			</flow-input>
			<div col>
	...
	renderEstimate() {
		...
		<div class="estimate-tx">
			<table>
				...
				${totalAmount ? html`<tr is="i18n-td"><td>Total Amount</td><td> ${KAS(totalAmount)} KOBRA</td></tr>` : ''}
			</table>
		</div>
		`
	}
	...
	async sendAfterConfirming() {
		...
		if (estimate.fee > this.alertFeeAmount) {
			let msg = i18n.t('Transaction Fee ([n] KOBRA) is too large.');
